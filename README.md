...
現時点で `【要編集】` の未確定項目はありません(企画名・開催日・対象商品・商品情報・代表者名・クーポンリンク、いずれも記入済み)。
今後、内容や画像を差し替える場合は `index.html` 内の `<!-- ▼〜 -->` コメントを目印にしてください。

## 実装方針

- 基本はJavaScript不使用(楽天GOLD/商品ページでの利用を想定し、HTML/CSSのみで構成)。ただし以下2箇所のみ、body末尾の<script>でJSを使用しています。JSが実行できない環境でも該当の演出が出ないだけでレイアウトは崩れません
  - FV看板画像のシャインエフェクト(ランダムな間隔で自然に光らせる)
  - 対象商品エリアの金の紙吹雪・リボン演出(ページ読み込み時に生成)
